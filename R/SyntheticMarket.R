#' A model-independent synthetic market
#'
...
SyntheticMarket <- function(design, firms, products, ownership, shares,
  ...
  structure(
    list(
      ...
      ownership = ownership,
      owner = ownership,
      shares = unname(shares),
      prices = unname(prices),
      reference_product = design$reference_product,
      ...
      observed_markup = if (is.list(observed)) {
        observed$reference_markup
      } else NULL,
